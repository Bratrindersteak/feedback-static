...
define('privateMessage', function(require, exports, module) {
    ...
    var WINDOW_WIDTH = vars.ScreenSize.split('x')[0]; // 设备可视宽度.
    ...
    var IMAGE_WIDTH = Math.floor(WINDOW_WIDTH * 0.53334 * 0.55 * vars.PixelRatio); // 图片宽度.
    ...
    var $instantMessageTemplate = $('#instantMessageTemplate');
    ...
    if (/my\.test\.56\.com|my\.test\.sohu\.com|127\.0\.0\.1|localhost/.test(URL_HOST) || LOCATION_PROTOCOL === 'file:') {
        ...
        hosts.realTimeRouter = vars.PROTOCOL + '10.16.14.184/user/private/api/router.do';
    ...
    var IMTemplateView = Bone.View.extend({ // 实时消息标签视图.
        template: Handlebars.compile($instantMessageTemplate.html()),
        render: function() {
            var template = this.getTemplate();
            var html = Bone.Renderer.render(template, this.model);
	...
	var ListView = Bone.View.extend({ // 私信列表视图.
        ...
        events: {
            ...
            'tap .to-portrait': 'tracePortrait' // 点击对方用户头像.
        },
        initialize: function() {
		    var self = this;
        ...
        checkPic: function(data) {
            ...
            this.$el.find('.picture .content img').each(function() {
                $(this).on('load', function() {
                    ...
                    if ((height / width) > 2 && width >= pWidth) {
        ...
        tapPicture: function(event) {
            ...
            var originSrc = $(event.currentTarget).attr('name');
        ...
        tracePortrait: function() {
            traceClick.pingback(null, 'sixin_touxiang_click', {
                remark: '私信会话页-点击用户头像'
            },function(){});
        }
    });
    ...
    var SendView = Bone.View.extend({ // 私信列表视图.
        ...
        events: {
            ...
            'tap #menuHome': 'traceToHome', // 菜单-点击用户主页.
            'tap #menuShop': 'traceToShop', // 菜单-点击商家店铺.
            'tap #menuFans': 'traceToFans' // 菜单-点击粉丝社区.
        },
        initialize: function() {
            this.listenTo(this.model, 'response', this.render);
        },
        render: function(data) {
            ...
            if (_.indexOf(imArr, data.messageId) !== -1) {
                return;
            }

            imArr.push(data.messageId);
        ...
        writing: function(event) {
            ...
            if ($messageList.height() > $wrapper.height()) {
                myScroll.scrollToElement(document.querySelector('#messageList .message-box:last-child'), 100, null, null);
            }
        },
        ...
        checkPic: function() {
            $messageList.children('.message-box').find('.picture .content img').on('load', function() {
                var pWidth = $(this).parent().width();
                var width = $(this).width();
                var height = $(this).height();
        ...
        traceSwitch: function() {
            traceClick.pingback(null, 'sixin_caidan_click', {
                remark: '私信会话页-点击切换菜单'
            },function(){});
        },
        traceUpload: function() {
            traceClick.pingback(null, 'sx_chatu_click', {
                remark: '私信会话页-点击发送图片'
            },function(){});
        },
        traceToHome: function() {
            traceClick.pingback(null, 'sixin_caidan_home', {
                remark: '私信会话页-菜单-点击用户主页'
            },function(){});
        },
        traceToShop: function() {
            traceClick.pingback(null, 'sixin_caidan_shop', {
                remark: '私信会话页-菜单-点击商家店铺'
            },function(){});
        },
        traceToFans: function() {
            traceClick.pingback(null, 'sixin_caidan_fans', {
                remark: '私信会话页-菜单-点击粉丝社区'
            },function(){});
        },
        traceClear: function() {
            traceClick.pingback(null, 'sixin_caidan_clear', {
                remark: '私信会话页-菜单-点击清空记录'
            },function(){});
        }
    });
    ...
    var DisplayLayerView = Bone.View.extend({ // 展示大图视图.
        ...
        initialize: function() {
            ...
            self.$el.find('img').attr('src', self.model);
            self.$el.show();
            self.open();
    ...
    var Presents = Bone.Object.extend({
        initialize: function (config) {
            ...
            iMTemplateView = new IMTemplateView();
        ...
        onMessage: function(data) {
            ...
            for (i = 0; i < data.length; i += 1) {
                ...
                if (_.indexOf(imArr, info.messageId) !== -1) {
                    continue;
                }

                imArr.push(info.messageId);

                iMTemplateView.model = info;
                $messageList.append(iMTemplateView.render());

                if (/^<img/.test(info.content)) {
                    $messageList.children('.message-box').find('.picture .content img').on('load', function() {
                        var pWidth = $(this).parent().width();
                        var width = $(this).width();
                        var height = $(this).height();
                    ...
                    if ($messageList.height() > $wrapper.height()) {
                        var imTimer = setTimeout(function() {
                            myScroll.refresh();
                            myScroll.scrollToElement(document.querySelector('#messageList .message-box:last-child'), 100, null, null);
                        }, SHOW_DELAY);
                    }
...
// application====================================================================================================
    ...
    Handlebars.registerHelper('messageContent', function (value) { // 私信消息内容.
        ...
        if (contentType == 1) {
            ...
            var img = new Image();
            var originSrc = value.content.split('"')[1];
            var src = value.content.split('"')[1].replace(/(\.jpg|\.png|\.gif)/, '/cut@m=resize,w=' + IMAGE_WIDTH + '$1');

            img.src = src;

            if ((img.height / img.width) > 2) {
                messageContent += '<div class="content"><div class="image" name="' + originSrc + '"><img src="' + img.src + '" width="auto" height="150" /></div></div>';
            } else {
                messageContent += '<div class="content"><div class="image" name="' + originSrc + '"><img src="' + img.src + '" width="100%" /></div></div>';
            }
    ...
    Handlebars.registerHelper('sendMessageContent', function (value) { // 发送私信消息内容.
        ...
        if (/^<img/.test(value)) {
            ...
            if ((img.height / img.width) > 2) {
                senderMessageContent += '<div class="content"><div class="image" name="' + originSrc + '"><img src="' + img.src + '" width="auto" height="150" /></div></div>';
            } else {
                senderMessageContent += '<div class="content"><div class="image" name="' + originSrc + '"><img src="' + img.src + '" width="100%" /></div></div>';
            }
        } else {
    ...
    Handlebars.registerHelper('isRelated', function (value, options) { // 实时消息过滤.

        if (value.senderUid == TO_UID && value.receiverUid == selfUid) {
            return options.fn(this);
        }

        if (value.senderUid == selfUid && value.receiverUid == TO_UID) {
            return options.fn(this);
        }
    });

    Handlebars.registerHelper('selfPic', function () { // 实时消息己方头像.
        return selfPicUrl;
    });

    Handlebars.registerHelper('toPic', function () { // 实时消息对方头像.
        return toPicUrl;
    });
